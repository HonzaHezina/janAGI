...
try:
    ...
    from .planner import _extract_first_json, validate_plan_json, plan_obj_to_text, build_decision_prompt, format_plan_preview
except ImportError:
    from models import ChatCompletionRequest, Message
    from providers import get_provider
    from config import logger, LLM_PROVIDER, DEFAULT_MODELS, AGENTS_DIR
...
# _extract_first_json moved to llm_mcp_app/planner.py


# validate_plan_json moved to llm_mcp_app/planner.py


# plan_obj_to_text moved to llm_mcp_app/planner.py
...
async def orchestrate_response(request: ChatCompletionRequest, agents: Dict[str, Agent]) -> JSONResponse:
    ...
    # python
    decision_prompt = build_decision_prompt(list(agents.keys()), user_message)
    ...
    try:
        ...
        if json_obj and isinstance(json_obj, dict) and "plan" in json_obj:
            ...
            plan_response = format_plan_preview(json_obj)
